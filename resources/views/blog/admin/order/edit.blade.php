...
@section('content')
    <section class="content-header">
        ...
        @if(!$order->status)
            <a href="{{ route('blog.admin.orders.change', $item->id) }}/?status=1" class="btn btn-success btn-xs">
                Одобрить</a>
            <a href="#" class="btn btn-warning btn-xs redact">Редактировать</a>
        @else
            <a href="{{ route('blog.admin.orders.change', $item->id) }}/?status=0" class="btn btn-default btn-xs">
                Вернуть на доработку</a>
        @endif

        <a href="" class="btn btn-xs">
            <form action="{{ route('blog.admin.orders.destroy', $item->id) }}" id="delform" method="post">
                @method('DELETE')
                @csrf
